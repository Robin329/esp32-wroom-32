...
esp_err_t InitRom(EEPROM_t *dev, i2c_port_t i2c_port)
{
	...
	WLOG("i2c_param_config=%d\n", ret);
	if (ret != ESP_OK)
		return ret;
	ret = i2c_driver_install(i2c_port, I2C_MODE_MASTER, 0, 0, 0);
	WLOG("i2c_driver_install=%d\n", ret);
	return ret;
}

static esp_err_t ReadReg8(EEPROM_t *dev, i2c_port_t i2c_port, int chip_addr,
			  uint8_t data_addr, uint8_t *data)
{
	i2c_cmd_handle_t cmd = i2c_cmd_link_create();
	i2c_master_start(cmd);
	i2c_master_write_byte(cmd, chip_addr << 1 | I2C_MASTER_WRITE,
			      ACK_CHECK_EN);
	i2c_master_write_byte(cmd, data_addr, ACK_CHECK_EN);
	i2c_master_start(cmd);
	i2c_master_write_byte(cmd, chip_addr << 1 | I2C_MASTER_READ,
			      ACK_CHECK_EN);
...
static esp_err_t WriteReg8(EEPROM_t *dev, i2c_port_t i2c_port, int chip_addr,
			   uint8_t data_addr, uint8_t data)
{
	i2c_cmd_handle_t cmd = i2c_cmd_link_create();
	i2c_master_start(cmd);
...
static esp_err_t ReadReg16(EEPROM_t *dev, i2c_port_t i2c_port, int chip_addr,
			   uint16_t data_addr, uint8_t *data)
{
...
static esp_err_t WriteReg16(EEPROM_t *dev, i2c_port_t i2c_port, int chip_addr,
			    uint16_t data_addr, uint8_t data)
{
	uint8_t high_addr = (data_addr >> 8) & 0xff;
	uint8_t low_addr = data_addr & 0xff;
...
esp_err_t ReadRom(EEPROM_t *dev, uint16_t data_addr, uint8_t *data)
{
	...
	if (dev->_kbits < 32) {
		...
		// WLOG("ReadRom _chip_addr=%#x _data_addr=%d\n", _chip_addr,
		//      _data_addr);
		return ReadReg8(dev, dev->_i2c_port, _chip_addr, _data_addr,
				data);
	} else {
		int _chip_addr = dev->_chip_addr;
		// WLOG("ReadRom _chip_addr=%#x _data_addr=%d\n", _chip_addr,
		//      data_addr);
		return ReadReg16(dev, dev->_i2c_port, _chip_addr, data_addr,
				 data);
	}
}
...
esp_err_t WriteRom(EEPROM_t *dev, uint16_t data_addr, uint8_t data)
{
	...
	if (dev->_kbits < 32) {
		...
		uint16_t _data_addr = data_addr - (blockNumber * 256);
		int _chip_addr = dev->_chip_addr + blockNumber;
		// WLOG("WriteRom _chip_addr=%#x _data_addr=%d\n", _chip_addr,
		//      _data_addr);
		return WriteReg8(dev, dev->_i2c_port, _chip_addr, _data_addr,
				 data);
	} else {
		int _chip_addr = dev->_chip_addr;
		// WLOG("WriteRom _chip_addr=%#x data_addr=%d\n", _chip_addr,
		//      data_addr);
		return WriteReg16(dev, dev->_i2c_port, _chip_addr, data_addr,
				  data);
	}
}
