...
static int do_i2cconfig_cmd(int argc, char **argv)
{
	...
	if (i2cconfig_args.port->count) {
		if (i2c_get_port(i2cconfig_args.port->ival[0], &i2c_port) !=
		    ESP_OK) {
			return 1;
		}
	}
...
{
	i2cconfig_args.port =
		arg_int0(NULL, "port", "<0|1>", "Set the I2C bus port number");
	i2cconfig_args.freq = arg_int0(NULL, "freq", "<Hz>",
				       "Set the frequency(Hz) of I2C bus");
	i2cconfig_args.sda =
		arg_int1(NULL, "sda", "<gpio>", "Set the gpio for I2C SDA");
	i2cconfig_args.scl =
		arg_int1(NULL, "scl", "<gpio>", "Set the gpio for I2C SCL");
	i2cconfig_args.end = arg_end(2);
	const esp_console_cmd_t i2cconfig_cmd = { .command = "i2cconfig",
						  .help = "Config I2C bus",
						  .hint = NULL,
	...
	for (int i = 0; i < 128; i += 16) {
		...
		for (int j = 0; j < 16; j++) {
			...
			esp_err_t ret = i2c_master_cmd_begin(
				i2c_port, cmd, 50 / portTICK_PERIOD_MS);
...
static void register_i2cget(void)
{
	...
	i2cget_args.register_address =
		arg_int0("r", "register", "<register_addr>",
...
static int do_i2cset_cmd(int argc, char **argv)
{
	...
	for (int i = 0; i < len; i++) {
		i2c_master_write_byte(cmd, i2cset_args.data->ival[i],
				      ACK_CHECK_EN);
	}
	i2c_master_stop(cmd);
	esp_err_t ret =
...
static int do_i2cdump_cmd(int argc, char **argv)
{
	...
	for (int i = 0; i < 128; i += 16) {
		...
		for (int k = 0; k < 16; k++) {
			...
			if ((block[k] & 0xff) == 0x00 ||
			    (block[k] & 0xff) == 0xff) {
				printf(".");
			} else if ((block[k] & 0xff) < 32 ||
				   (block[k] & 0xff) >= 127) {
				printf("?");
			} else {
...
static void register_i2cdump(void)
{
	i2cdump_args.chip_address =
		arg_int1("c", "chip", "<chip_addr>",
			 "Specify the address of the chip on that bus");
	i2cdump_args.size = arg_int0("s", "size", "<size>",
				     "Specify the size of each read");
